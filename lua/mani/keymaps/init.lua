...
keymap("n", "<leader>bi", ":Telescope builtin<cr>", keymap_description("open [B]uilt [I]n"))
...
--Insert mode
--codeium
vim.keymap.set('i', '<Right>', function() return vim.fn['codeium#Accept']() end,
    { desc = "[C]odeium [A]ccept", expr = true })
vim.keymap.set('i', '<C-l>', function() return vim.fn['codeium#CycleCompletions'](1) end,
    { desc = "[C]odeium next in the list [L]->{vim notions}", expr = true })
vim.keymap.set('i', '<C-h>', function() return vim.fn['codeium#CycleCompletions'](-1) end,
    { desc = "[C]odeium previous in the list [H]->{vim notions}", expr = true })
