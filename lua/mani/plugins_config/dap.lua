...
vim.keymap.set('n', '<Leader>dtb', function() require('dap').toggle_breakpoint() end,
    keymap_description("[D]ebug [T]oggle [B]reakpoint"))
vim.keymap.set('n', '<Leader>dsb', function() require('dap').set_breakpoint() end,
    keymap_description("[D]ebug [S]et [B]reakpoint"))
vim.keymap.set('n', '<Leader>dlp',
    function() require('dap').set_breakpoint(nil, nil, vim.fn.input('Log point message: ')) end,
    keymap_description("[D]ebug [L]og [P]oint"))
...
vim.keymap.set({ 'n', 'v' }, '<Leader>dh', function() require('dap.ui.widgets').hover() end,
    keymap_description("[D]ebug [H]over"))
vim.keymap.set({ 'n', 'v' }, '<Leader>dp', function() require('dap.ui.widgets').preview() end,
    keymap_description("[D]ebug [P]review"))
vim.keymap.set('n', '<Leader>df',
    function()
        local widgets = require('dap.ui.widgets')
        widgets.centered_float(widgets.frames)
    end, keymap_description("[D]ebug [F]rames"))
vim.keymap.set('n', '<Leader>ds',
    function()
        local widgets = require('dap.ui.widgets')
        widgets.centered_float(widgets.scopes)
    end, keymap_description("[D]ebug [S]copes"))
...
dap.configurations.java = {
    {
        ...
        hostName = 'localhost',
        port = 5005 -- Replace <port_value> with the desired port value
    }
}
